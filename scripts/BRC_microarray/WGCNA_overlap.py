...
def compute_overlap(path="../data/BRC_microarray/", dataset="PPI"):
    Netherlands_clusters = np.genfromtxt("{}output/Netherlands/{}.final_features_WGCNA.txt".format(path, dataset), dtype=np.dtype(np.int))
    USA_clusters = np.genfromtxt("{}output/USA/{}.final_features_WGCNA.txt".format(path, dataset), dtype=np.dtype(np.int))
    
    Netherlands_proteins = []
    for i in Netherlands_clusters:
        i= int(i)
        with open("{}output/Netherlands/{}.clusters_WGCNA.txt".format(path, dataset), encoding="utf-8") as lines:
    ...
    for j in USA_clusters:
        j=int(j)
        with open("{}output/USA/{}.clusters_WGCNA.txt".format(path, dataset), encoding="utf-8") as lines:
