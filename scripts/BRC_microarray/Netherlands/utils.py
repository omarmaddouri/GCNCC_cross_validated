...
def prepare_enrichment_clusters(dataset="PPI"):
    print("Preparing enrichment analysis data for PPI network ...")
    Names = np.genfromtxt("../../data/BRC_microarray/input/mart_gene_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_names = {}
    for i in range(Names.shape[0]):
        gene_names[Names[i,0]]=Names[i,1]
    
    protein_name = np.genfromtxt("../../data/BRC_microarray/input/protein_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    protein_name_map = {}
    for i in range(protein_name.shape[0]):
        protein_name_map[protein_name[i,0]]=protein_name[i,1]
        
            
    Ids = np.genfromtxt("../../data/BRC_microarray/output/Netherlands/PPI.ids.txt", dtype=np.dtype(str), delimiter="\t")
    protein_ids = {}
    for i in range(Ids.shape[0]):
        protein_ids[Ids[i,1]]=Ids[i,0]
        
    Clusters = open("../../data/BRC_microarray/output/Netherlands/PPI.clusters.txt", encoding="utf-8")
    with open("../../data/BRC_microarray/output/Netherlands/PPI.enrichment.txt", "w", newline='', encoding="utf-8") as f:
        ...
        for line in Clusters:
            ...
            for i in range(len(columns)-1):
                if(dataset == "PPI"):
                    if(protein_ids[columns[i+1]] in protein_name_map.keys()):
                        cl.append(protein_name_map[protein_ids[columns[i+1]]])
                    else:
                        cl.append(protein_ids[columns[i+1]])
                else:
                    cl.append(protein_name_map[protein_ids[columns[i+1]]])
...
def prepare_enrichment_clusters_without_GCN(dataset="PPI"):
    ...
    Clusters = open("../../data/BRC_microarray/output/Netherlands/PPI.clusters_without_GCN.txt", encoding="utf-8")
    with open("../../data/BRC_microarray/output/Netherlands/PPI.enrichment_without_GCN.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def prepare_enrichment_clusters_without_GCN_and_local_modules(dataset="PPI"):
    ...
    Clusters = open("../../data/BRC_microarray/output/Netherlands/PPI.clusters_without_GCN_and_local_modules.txt", encoding="utf-8")
    with open("../../data/BRC_microarray/output/Netherlands/PPI.enrichment_without_GCN_and_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def prepare_enrichment_clusters_without_local_modules(dataset="PPI"):
    ...
    Clusters = open("../../data/BRC_microarray/output/Netherlands/PPI.clusters_without_local_modules.txt", encoding="utf-8")
    with open("../../data/BRC_microarray/output/Netherlands/PPI.enrichment_without_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def get_GO_labels(labels):    
    ...
    GO = np.genfromtxt("../../data/BRC_microarray/input/mart_go.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_go_domain = {}
...
def prepare_entrez_ensembl_mapping():
    ...
    MAP = np.genfromtxt("../../data/BRC_microarray/input/mart_entrez_ensembl.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    Ensembl_Entrez = {}
    for i in range(MAP.shape[0]):
        if (MAP[i,1] != ""):
            Ensembl_Entrez[MAP[i,1]] = MAP[i,0]
    with open("../../data/BRC_microarray/output/Netherlands/PPI.ensembl.entrez.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_PPI_gene_expression():
    ...
    #Read PPI network
    PPI = np.genfromtxt("../../data/BRC_microarray/input/9606.protein.links.v10.5.txt", skip_header=1, dtype=np.dtype(str))
    protein_ids = {}
    p_id = 0
    ...
    for i in range(PPI.shape[0]):
        ...
        if(ENSP not in protein_ids):
            ...
            p_id+=1
    with open("../../data/BRC_microarray/output/Netherlands/PPI.ids.txt", "w", newline='', encoding="utf-8") as f:
        w_indices = csv.writer(f, delimiter ='\t')
        for key, val in protein_ids.items():
            w_indices.writerow([key, val])
    #Map Gene IDs to Protein IDs
    MAP = np.genfromtxt("../../data/BRC_microarray/input/gene_protein.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    Gene_Protein_MAP = defaultdict(list)
    for i in range(MAP.shape[0]):
        if(MAP[i][0] != "" and MAP[i][1] != ""):
            Gene_Protein_MAP[MAP[i][0]].append(MAP[i][1])    
    ...
    with open("../../data/BRC_microarray/output/Netherlands/PPI.adjacency.matrix", "w", newline='', encoding="utf-8") as f:        
        w_adj = csv.writer(f, delimiter ='\t')
        for key, val in adj_matrix.items():
            for neighbor in val:
                w_adj.writerow([key, neighbor])
    #Prepare gene expression features    
    GE_matrix = np.zeros((len(protein_ids), 296), dtype=object) #Initialize gene expression values to 0's
    ...
    gene_names = {}
    for i in range(Names.shape[0]):
        gene_names[Names[i,1]]=Names[i,0]
        
    GE = np.genfromtxt("../../data/BRC_microarray/input/Netherlands/gene_expression/Table_NKI_295_1.txt", skip_header=2, dtype=np.dtype(str), delimiter="\t")
    feature_indices = []
    index = 2
    while (index < GE.shape[1]):
        feature_indices.append(index)
        index = index+5
    start = 1                   
    for k in range(GE.shape[0]):
        if(GE[k][1] in gene_names.keys()):
            key = gene_names[GE[k][1]]
            if( key in Gene_Protein_MAP.keys() ):
                for protein in Gene_Protein_MAP[key]:
                    if(protein in protein_ids.keys()):
                        GE_matrix[protein_ids[protein], start:start+len(feature_indices)] = GE[k, feature_indices]
    
    start = start+len(feature_indices)         
    GE = np.genfromtxt("../../data/BRC_microarray/input/Netherlands/gene_expression/Table_NKI_295_2.txt", skip_header=2, dtype=np.dtype(str), delimiter="\t")
    feature_indices = []
    index = 2
    while (index < GE.shape[1]):
        feature_indices.append(index)
    ...
    start = start+len(feature_indices)         
    GE = np.genfromtxt("../../data/BRC_microarray/input/Netherlands/gene_expression/Table_NKI_295_3.txt", skip_header=2, dtype=np.dtype(str), delimiter="\t")
    feature_indices = []
    index = 2
    while (index < GE.shape[1]):
        feature_indices.append(index)
    ...
    start = start+len(feature_indices)         
    GE = np.genfromtxt("../../data/BRC_microarray/input/Netherlands/gene_expression/Table_NKI_295_4.txt", skip_header=2, dtype=np.dtype(str), delimiter="\t")
    feature_indices = []
    index = 2
    while (index < GE.shape[1]):
        feature_indices.append(index)
    ...
    start = start+len(feature_indices)         
    GE = np.genfromtxt("../../data/BRC_microarray/input/Netherlands/gene_expression/Table_NKI_295_5.txt", skip_header=2, dtype=np.dtype(str), delimiter="\t")
    feature_indices = []
    index = 2
    while (index < GE.shape[1]):
        feature_indices.append(index)
    ...
    start = start+len(feature_indices)         
    GE = np.genfromtxt("../../data/BRC_microarray/input/Netherlands/gene_expression/Table_NKI_295_6.txt", skip_header=2, dtype=np.dtype(str), delimiter="\t")
    feature_indices = []
    index = 2
    while (index < GE.shape[1]):
        feature_indices.append(index)
    ...
    with open("../../data/BRC_microarray/output/Netherlands/PPI.GE_Features.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_clusters_AP(labels, cluster_centers_indices):
    ...
    with open("../../data/BRC_microarray/output/Netherlands/PPI.clusters.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_clusters_AP_without_GCN(labels, cluster_centers_indices):
    ...
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/BRC_microarray/output/Netherlands/PPI.clusters_without_GCN.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def prepare_clusters_AP_without_GCN_and_local_modules(labels, cluster_centers_indices):
    ...
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/BRC_microarray/output/Netherlands/PPI.clusters_without_GCN_and_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def prepare_clusters_AP_without_local_modules(labels, cluster_centers_indices):
    ...
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/BRC_microarray/output/Netherlands/PPI.clusters_without_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def get_clinical_status_Netherlands():
    
    clinical = np.genfromtxt("../../data/BRC_microarray/input/Netherlands/clinical/clinical.txt", skip_header=1, delimiter="\t", dtype=np.dtype(str))
    clinical_features = list(map(int, clinical[:, 3]))
