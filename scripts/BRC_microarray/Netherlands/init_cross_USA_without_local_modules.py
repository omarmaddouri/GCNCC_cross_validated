...
clusters = open("{}{}.clusters_without_local_modules.txt".format(path, dataset), encoding="utf-8")

# Plotting of ROC/AUC
nb_columns = len(labels)

clust = np.genfromtxt("{}{}.final_features_without_local_modules.txt".format(path, dataset), dtype=np.dtype(np.int))
...
plt.title('BREAST CANCER: Netherlands-USA (without local modules)')
