...
dataset="PPI"
features = np.genfromtxt("../../data/BRC_microarray/output/USA/{}.GE_Features.txt".format(dataset), dtype=np.dtype(np.float32))
labels = get_clinical_status_USA()
clusters = open("{}{}.clusters_without_GCN_and_local_modules.txt".format(path, dataset), encoding="utf-8")

# Plotting of ROC/AUC
nb_columns = len(labels)

clust = np.genfromtxt("{}{}.final_features_without_GCN_and_local_modules.txt".format(path, dataset), dtype=np.dtype(np.int))
...
plt.title('BREAST CANCER: Netherlands-USA (without GCN and local modules)')
