...
def compute_overlap(path1="../data/SCZ_microarray/", path2="../data/SCZ_RNAseq/", dataset="PPI"):
    GSE_clusters = np.genfromtxt("{}output/GSE/{}.final_features_WGCNA.txt".format(path1, dataset), dtype=np.dtype(np.int))
    syn4590909_clusters = np.genfromtxt("{}output/syn4590909/{}.final_features_WGCNA.txt".format(path2, dataset), dtype=np.dtype(np.int))
    syn2759792_clusters = np.genfromtxt("{}output/syn2759792/{}.final_features_WGCNA.txt".format(path2, dataset), dtype=np.dtype(np.int))
    
    GSE_proteins = []
    for i in GSE_clusters:
        i= int(i)
        with open("{}output/GSE/{}.clusters_WGCNA.txt".format(path1, dataset), encoding="utf-8") as lines:
    ...
    for j in syn4590909_clusters:
        j=int(j)
        with open("{}output/syn4590909/{}.clusters_WGCNA.txt".format(path2, dataset), encoding="utf-8") as lines:
    ...
    for k in syn2759792_clusters:
        k=int(k)
        with open("{}output/syn2759792/{}.clusters_WGCNA.txt".format(path2, dataset), encoding="utf-8") as lines:
