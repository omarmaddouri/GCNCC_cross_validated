...
def run_AP_clustering(path, dataset, features, labels):
    ...
    '''
    tmp2 = np.matmul(tmp1,A1)#A3
    tmp = tmp2.copy()
    tmp[tmp!=0]=1
    mask = np.maximum(mask, tmp)
    
    tmp1 = np.matmul(tmp2,A1)#A4
    tmp = tmp1.copy()
    tmp[tmp!=0]=1
    mask = np.maximum(mask, tmp)
    
    tmp2 = np.matmul(tmp1,A1)#A5
    tmp = tmp2.copy()
    tmp[tmp!=0]=1
    mask = np.maximum(mask, tmp)
    '''
...
def prepare_enrichment_clusters(dataset="PPI"):
    print("Preparing enrichment analysis data for PPI network ...")
    Names = np.genfromtxt("../../data/SCZ_RNAseq/input/mart_gene_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_names = {}
    for i in range(Names.shape[0]):
        gene_names[Names[i,0]]=Names[i,1]
    
    protein_name = np.genfromtxt("../../data/SCZ_RNAseq/input/protein_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    protein_name_map = {}
    for i in range(protein_name.shape[0]):
        protein_name_map[protein_name[i,0]]=protein_name[i,1]
        
            
    Ids = np.genfromtxt("../../data/SCZ_RNAseq/output/syn4590909/PPI.ids.txt", dtype=np.dtype(str), delimiter="\t")
    gene_ids = {}
    for i in range(Ids.shape[0]):
        gene_ids[Ids[i,1]]=Ids[i,0]
        
    Clusters = open("../../data/SCZ_RNAseq/output/syn4590909/PPI.clusters.txt", encoding="utf-8")
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.enrichment.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_enrichment_clusters_without_GCN(dataset="PPI"):
    ...
    Clusters = open("../../data/SCZ_RNAseq/output/syn4590909/PPI.clusters_without_GCN.txt", encoding="utf-8")
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.enrichment_without_GCN.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def prepare_enrichment_clusters_without_GCN_and_local_modules(dataset="PPI"):
    ...
    Clusters = open("../../data/SCZ_RNAseq/output/syn4590909/PPI.clusters_without_GCN_and_local_modules.txt", encoding="utf-8")
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.enrichment_without_GCN_and_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def prepare_enrichment_clusters_without_local_modules(dataset="PPI"):
    ...
    Clusters = open("../../data/SCZ_RNAseq/output/syn4590909/PPI.clusters_without_local_modules.txt", encoding="utf-8")
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.enrichment_without_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def get_GO_labels(labels):    
    ...
    GO = np.genfromtxt("../../data/SCZ_RNAseq/input/mart_go.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_go_domain = {}
...
def prepare_entrez_ensembl_mapping():
    ...
    MAP = np.genfromtxt("../../data/SCZ_RNAseq/input/mart_entrez_ensembl.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    Ensembl_Entrez = {}
    for i in range(MAP.shape[0]):
        if (MAP[i,1] != ""):
            Ensembl_Entrez[MAP[i,1]] = MAP[i,0]
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.ensembl.entrez.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_PPI_gene_expression():
    ...
    #Read PPI network
    PPI = np.genfromtxt("../../data/SCZ_RNAseq/input/9606.protein.links.v10.5.txt", skip_header=1, dtype=np.dtype(str))
    protein_ids = {}
    p_id = 0
    ...
    for i in range(PPI.shape[0]):
        ...
        if(ENSP not in protein_ids):
            ...
            p_id+=1
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.ids.txt", "w", newline='', encoding="utf-8") as f:
        w_indices = csv.writer(f, delimiter ='\t')
        for key, val in protein_ids.items():
            w_indices.writerow([key, val])
    #Map Gene IDs to Protein IDs
    MAP = np.genfromtxt("../../data/SCZ_RNAseq/input/gene_protein.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    ...
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.adjacency.matrix", "w", newline='', encoding="utf-8") as f:        
        w_adj = csv.writer(f, delimiter ='\t')
        for key, val in adj_matrix.items():
            for neighbor in val:
                w_adj.writerow([key, neighbor])
    #Prepare gene expression features    
    GE = np.genfromtxt("../../data/SCZ_RNAseq/input/syn4590909/RNAseq_SCZ_BD_GVEX_datExpr.csv", skip_header=1, skip_footer=5, dtype=np.dtype(str), delimiter=",")
    ...
    with open("../../data/SCZ_RNAseq/input/syn4590909/RNAseq_SCZ_BD_GVEX_datExpr.csv", encoding="utf-8") as lines:
        samples = np.genfromtxt(islice(lines, 0, 1), dtype=np.dtype(str), delimiter=",")
    samples = samples[1:]
    for p in range(len(samples)):
        if(samples[p] in Sample_diagnosis.keys()):
            samples[p] = Sample_diagnosis[samples[p]]
    indices = np.where(np.logical_or(samples == "SCZ", samples == "Control"))
    ...
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.GE_Features.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_clusters_AP(labels, cluster_centers_indices):
    ...
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.clusters.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_clusters_AP_without_GCN(labels, cluster_centers_indices):
    ...
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.clusters_without_GCN.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def prepare_clusters_AP_without_GCN_and_local_modules(labels, cluster_centers_indices):
    ...
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.clusters_without_GCN_and_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def prepare_clusters_AP_without_local_modules(labels, cluster_centers_indices):
    ...
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/SCZ_RNAseq/output/syn4590909/PPI.clusters_without_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def load_data(path, dataset, features, status_labels):
    """Load dataset network with SCZ_RNAseq dataset """
