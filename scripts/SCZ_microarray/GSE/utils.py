...
def get_top_clusters_without_network(path, dataset, features, labels, clusters):
    clusters.seek(0)
    num_lines = sum(1 for line in clusters)
    full_results = {}
    results = {}
    ...
    while(index < len(sorted_results)):
        ...
        #size cluster between 1 and 50 and connectivity validated, and their P-value is less than 0.1
        #and (full_results[current_cluster][2] < 50)
        if( (full_results[current_cluster][0] >= 2) ):
            top_clusters.append(current_cluster)
            index+=1
        else:
            index+=1
...
def run_AP_clustering_without_GCN_and_local_modules(path, dataset, features, labels):
    _, A, _ = load_data(path, dataset, features, labels)
    ...
    print("Preparing clustering results ...")
    prepare_clusters_AP_without_GCN_and_local_modules(labels, cluster_centers_indices)
    print("Clustering results file ready")
    print("Preparing enrichment analysis data for PPI network ...")
    prepare_enrichment_clusters_without_GCN_and_local_modules(dataset="PPI")
    print("Enrichment analysis data ready")
...
def prepare_enrichment_clusters(dataset="PPI"):
    print("Preparing enrichment analysis data for PPI network ...")
    Names = np.genfromtxt("../../data/SCZ_microarray/input/mart_gene_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_names = {}
    for i in range(Names.shape[0]):
        gene_names[Names[i,0]]=Names[i,1]
    
    protein_name = np.genfromtxt("../../data/SCZ_microarray/input/protein_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    protein_name_map = {}
    for i in range(protein_name.shape[0]):
        protein_name_map[protein_name[i,0]]=protein_name[i,1]
        
            
    Ids = np.genfromtxt("../../data/SCZ_microarray/output/GSE/PPI.ids.txt", dtype=np.dtype(str), delimiter="\t")
    gene_ids = {}
    for i in range(Ids.shape[0]):
        gene_ids[Ids[i,1]]=Ids[i,0]
        
    Clusters = open("../../data/SCZ_microarray/output/GSE/PPI.clusters.txt", encoding="utf-8")
    with open("../../data/SCZ_microarray/output/GSE/PPI.enrichment.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_enrichment_clusters_without_GCN(dataset="PPI"):
    print("Preparing enrichment analysis data for PPI network ...")
    Names = np.genfromtxt("../../data/SCZ_microarray/input/mart_gene_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_names = {}
    for i in range(Names.shape[0]):
    ...
    Clusters = open("../../data/SCZ_microarray/output/GSE/PPI.clusters_without_GCN.txt", encoding="utf-8")
    with open("../../data/SCZ_microarray/output/GSE/PPI.enrichment_without_GCN.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def prepare_enrichment_clusters_without_GCN_and_local_modules(dataset="PPI"):
    print("Preparing enrichment analysis data for PPI network ...")
    Names = np.genfromtxt("../../data/SCZ_microarray/input/mart_gene_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_names = {}
    for i in range(Names.shape[0]):
    ...
    Clusters = open("../../data/SCZ_microarray/output/GSE/PPI.clusters_without_GCN_and_local_modules.txt", encoding="utf-8")
    with open("../../data/SCZ_microarray/output/GSE/PPI.enrichment_without_GCN_and_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def prepare_enrichment_clusters_without_local_modules(dataset="PPI"):
    print("Preparing enrichment analysis data for PPI network ...")
    Names = np.genfromtxt("../../data/SCZ_microarray/input/mart_gene_names.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_names = {}
    for i in range(Names.shape[0]):
    ...
    Clusters = open("../../data/SCZ_microarray/output/GSE/PPI.clusters_without_local_modules.txt", encoding="utf-8")
    with open("../../data/SCZ_microarray/output/GSE/PPI.enrichment_without_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_cluster = csv.writer(f, delimiter ='\t')
        for line in Clusters:
            line = line.strip()
            columns = line.split("\t")
...
def get_GO_labels(labels):    
    Ids = np.genfromtxt("../../data/SCZ_microarray/output/GSE/PPI.ids.txt", dtype=np.dtype(str), delimiter="\t")
    gene_ids = {}
    for i in range(Ids.shape[0]):
        gene_ids[Ids[i,1]]=Ids[i,0]
        
    GO = np.genfromtxt("../../data/SCZ_microarray/input/mart_go.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    gene_go_domain = {}
    for i in range(GO.shape[0]):
        if(GO[i,2] != ""):
            gene_go_domain[GO[i,0]] = GO[i,2]
    
    for i in range(len(labels)):
        key = gene_ids[str(i)]
        if(key in gene_go_domain.keys()):
            labels[i] = gene_go_domain[key]
        
    return labels

def prepare_entrez_ensembl_mapping():
    print("Mapping Entrez to Ensembl ids for PPI network ...")
    MAP = np.genfromtxt("../../data/SCZ_microarray/input/mart_entrez_ensembl.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    Ensembl_Entrez = {}
    for i in range(MAP.shape[0]):
        if (MAP[i,1] != ""):
            Ensembl_Entrez[MAP[i,1]] = MAP[i,0]
    with open("../../data/SCZ_microarray/output/GSE/PPI.ensembl.entrez.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_PPI_gene_expression():
    ...
    #Read PPI network
    PPI = np.genfromtxt("../../data/SCZ_microarray/input/9606.protein.links.v10.5.txt", skip_header=1, dtype=np.dtype(str))
    protein_ids = {}
    p_id = 0
    ...
    for i in range(PPI.shape[0]):
        ...
        if(ENSP not in protein_ids):
            ...
            p_id+=1
    with open("../../data/SCZ_microarray/output/GSE/PPI.ids.txt", "w", newline='', encoding="utf-8") as f:
        w_indices = csv.writer(f, delimiter ='\t')
        for key, val in protein_ids.items():
            w_indices.writerow([key, val])
            
    #Map Gene IDs to Protein IDs
    MAP = np.genfromtxt("../../data/SCZ_microarray/input/probes_protein.txt", skip_header=1, dtype=np.dtype(str), delimiter="\t")
    Probe_U133A_Protein_MAP = defaultdict(list)
    Probe_U133_Plus2_Protein_MAP = defaultdict(list)
    for i in range(MAP.shape[0]):
        if(MAP[i][1] != "" and MAP[i][2] != ""):
            Probe_U133A_Protein_MAP[MAP[i][1]].append(MAP[i][2])
        if(MAP[i][0] != "" and MAP[i][2] != ""):
            Probe_U133_Plus2_Protein_MAP[MAP[i][0]].append(MAP[i][2])    
    ...
    with open("../../data/SCZ_microarray/output/GSE/PPI.adjacency.matrix", "w", newline='', encoding="utf-8") as f:
    ...
    #Prepare gene expression features of GSE12649
    GE = np.genfromtxt("../../data/SCZ_microarray/input/GSE/GSE12649_series_matrix.txt", skip_header=70, skip_footer=1, dtype=np.dtype(str), delimiter="\t")
    GE[GE==""] = 0 #Replace missing values by 0
    
    with open("../../data/SCZ_microarray/input/GSE/GSE12649_series_matrix.txt", encoding="utf-8") as lines:
        clinical = np.genfromtxt(islice(lines, 40, 41), delimiter="\t", dtype=np.dtype(str))
    
    for i in range(len(clinical)):
        if(clinical[i].find("control") != -1):
            clinical[i] = "control"
        elif(clinical[i].find("schizophrenia") != -1):
            clinical[i] = "schizophrenia"
            
    indices = np.where(np.logical_or(clinical == "schizophrenia", clinical == "control"))
    adjusted_indices = [y for x in indices for y in x]
    
    GE_matrix_1 = np.zeros((len(protein_ids), len(adjusted_indices)+1), dtype=object) #Initialize gene expression values to 0's
    for j in range(GE_matrix_1.shape[0]):
        GE_matrix_1[j][0] = j #First column contains gene ID's     
        ...
        if( key in Probe_U133A_Protein_MAP.keys() ):
            for protein in Probe_U133A_Protein_MAP[key]:
                if(protein in protein_ids.keys()):
                    GE_matrix_1[protein_ids[protein], 1:] = GE[k, adjusted_indices]
    
    
    #Prepare gene expression features of GSE21138
    GE = np.genfromtxt("../../data/SCZ_microarray/input/GSE/GSE21138_series_matrix.txt", skip_header=73, skip_footer=1, dtype=np.dtype(str), delimiter="\t")
    GE[GE==""] = 0 #Replace missing values by 0
    
    with open("../../data/SCZ_microarray/input/GSE/GSE21138_series_matrix.txt", encoding="utf-8") as lines:
        clinical = np.genfromtxt(islice(lines, 43, 44), delimiter="\t", dtype=np.dtype(str))
    for i in range(len(clinical)):
        if(clinical[i].find("control") != -1):
            clinical[i] = "control"
        elif(clinical[i].find("schizophrenia") != -1):
    ...
    GE_matrix_2 = np.zeros((len(protein_ids), len(adjusted_indices)), dtype=object) #Initialize gene expression values to 0's    
    for k in range(GE.shape[0]):
        key = GE[k][0]
        key = key.replace('"', '')
        if( key in Probe_U133_Plus2_Protein_MAP.keys() ):
            for protein in Probe_U133_Plus2_Protein_MAP[key]:
                if(protein in protein_ids.keys()):
                    GE_matrix_2[protein_ids[protein], :] = GE[k, adjusted_indices]
                    
                    
    #Prepare gene expression features of GSE53987
    GE = np.genfromtxt("../../data/SCZ_microarray/input/GSE/GSE53987_series_matrix.txt", skip_header=67, skip_footer=1, dtype=np.dtype(str), delimiter="\t")
    GE[GE==""] = 0 #Replace missing values by 0
    
    with open("../../data/SCZ_microarray/input/GSE/GSE53987_series_matrix.txt", encoding="utf-8") as lines:
        clinical = np.genfromtxt(islice(lines, 34, 35), delimiter="\t", dtype=np.dtype(str))
    for i in range(len(clinical)):
        if(clinical[i].find("control") != -1):
            clinical[i] = "control"
        elif(clinical[i].find("schizophrenia") != -1):
    ...
    GE_matrix_3 = np.zeros((len(protein_ids), len(adjusted_indices)), dtype=object) #Initialize gene expression values to 0's     
    for k in range(GE.shape[0]):
        key = GE[k][0]
        key = key.replace('"', '')
        if( key in Probe_U133_Plus2_Protein_MAP.keys() ):
            for protein in Probe_U133_Plus2_Protein_MAP[key]:
                if(protein in protein_ids.keys()):
                    GE_matrix_3[protein_ids[protein], :] = GE[k, adjusted_indices]
                    
    for i in range(GE_matrix_1.shape[0]):
        if(np.count_nonzero(GE_matrix_1[i,1:].astype(np.float32))>0):
            GE_matrix_1[i,1:] = stats.zscore(GE_matrix_1[i,1:].astype(np.float32))
            
    for i in range(GE_matrix_2.shape[0]):
        if(np.count_nonzero(GE_matrix_2[i,:].astype(np.float32))>0):
            GE_matrix_2[i,:] = stats.zscore(GE_matrix_2[i,:].astype(np.float32))
            
    for i in range(GE_matrix_3.shape[0]):
        if(np.count_nonzero(GE_matrix_3[i,:].astype(np.float32))>0):
            GE_matrix_3[i,:] = stats.zscore(GE_matrix_3[i,:].astype(np.float32))
                              
    GE_matrix = np.hstack((GE_matrix_1, GE_matrix_2, GE_matrix_3))
    ...
    with open("../../data/SCZ_microarray/output/GSE/PPI.GE_Features.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_clusters_AP(labels, cluster_centers_indices):
    ...
    with open("../../data/SCZ_microarray/output/GSE/PPI.clusters.txt", "w", newline='', encoding="utf-8") as f:
...
def prepare_clusters_AP_without_GCN(labels, cluster_centers_indices):
    clusters = defaultdict(list)
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/SCZ_microarray/output/GSE/PPI.clusters_without_GCN.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def prepare_clusters_AP_without_GCN_and_local_modules(labels, cluster_centers_indices):
    clusters = defaultdict(list)
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/SCZ_microarray/output/GSE/PPI.clusters_without_GCN_and_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def prepare_clusters_AP_without_local_modules(labels, cluster_centers_indices):
    clusters = defaultdict(list)
    for i in range(len(labels)):
        clusters["Center_{}".format(cluster_centers_indices[labels[i]])].append(i)
    with open("../../data/SCZ_microarray/output/GSE/PPI.clusters_without_local_modules.txt", "w", newline='', encoding="utf-8") as f:
        w_clusters = csv.writer(f, delimiter ='\t')
        for key, val in clusters.items():
            line = []
            line.append(key)
...
def get_clinical_status_syn4590909():
...
def get_clinical_status_syn2759792():
    
    GE_clinical = np.genfromtxt("../../data/SCZ_RNAseq/input/syn2759792/CMC_MSSM-Penn-Pitt_Clinical.csv", skip_header=1, dtype=np.dtype(str), delimiter=",")
    Sample_diagnosis = {}
    for i in range(GE_clinical.shape[0]):
        Sample_diagnosis[GE_clinical[i][16]] = GE_clinical[i][2]
    with open("../../data/SCZ_RNAseq/input/syn2759792/CMC_MSSM-Penn-Pitt_DLPFC_mRNA_IlluminaHiSeq2500_geneExpressionRaw.tsv", encoding="utf-8") as lines:
        samples = np.genfromtxt(islice(lines, 0, 1), delimiter="\t", dtype=np.dtype(str))    
    clinical_features = []    
    for j in range(len(samples)):
        if(samples[j] in Sample_diagnosis.keys()):
...
def get_clinical_status_microarray():
    ...
    for j in range(len(clinical)):
        if(clinical[j].find("schizophrenia") != -1):
            clinical_features.append(1)
        elif(clinical[j].find("control") != -1):
            clinical_features.append(0)
...
def load_data(path, dataset, features, status_labels):
    """Load dataset network with SCZ_microarray dataset """
